...
Phase 3 — Smart Build: `cargo build --workspace`. Fix compile errors iteratively (max 3 rounds). Escalate if still broken.
Phase 4 — Clippy: `cargo clippy --workspace --all-targets --all-features -- -D warnings`. Fix warnings.
Phase 5 — Tests: `cargo test --workspace --all-targets`. Report failures.
Phase 6 — Print the Pull & Build Report summary.
