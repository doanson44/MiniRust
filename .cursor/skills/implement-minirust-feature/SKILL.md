...
User request
  ...
  → new adapter crate                  only when talking to SQL, Redis, Telegram, etc.
```
...
- If SQL Server or Redis is needed, keep startup working when the service is unset unless the feature cannot run without it.
- Redis: TTL on every entry; never store permanent business data there.
